...
const AddMedicineScreen = ({ navigation, route }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Step 2: Create the schedule linked to this medicine
      ...
      await api.post(`/schedules/user/${userInfo.id}/medicine/${medicineId}`, schedulePayload);
  ...
  return (
    <View style={styles.container}>
      <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
        ...
        {/* Search Results */}
        {searchLoading && <ActivityIndicator size="small" color="#3498db" style={{ marginVertical: 8 }} />}
        {searchError && !searchLoading && (
          <Text style={{ color: '#e74c3c', fontSize: 12, marginTop: 4, marginLeft: 4 }}>
            ⚠️ Search unavailable — you can still type the name manually
          </Text>
        )}
...
const styles = StyleSheet.create({
  ...
  suggestionsContainer: {
    ...
    borderColor: '#e0e6ed', maxHeight: 200, marginTop: 4,
  },
  suggestionItem: {
